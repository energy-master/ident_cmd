...
version = 1.1
print (f"SEntropyTemporalPersistent [{version}]")
...
class SEntropyTemporalPersistent(ConditionalRoot):
  ...
  def __str__(self):
    ...
    data = {
        "decision type" : "SEntropyTemporalPersistent",
  ...
  def run(self, data = {}):
    ...
    # get entropy and add to tracker
    # fourier_e_pivot = 0.0
    ...
    # print (t_index_arr)
    sentropy = -1
    sentropy = derived_data.discretised_spectral_entropy[discrete_data_idx]
    ...
    if geneInit:
        ...
        delta_f = 0
        delta_f = abs(sentropy - avg_energy)
        delta_f_pc = (delta_f / max(sentropy,avg_energy))  * 100
